...
export default function (pi: ExtensionAPI) {
  ...
  function startArm(owner: SessionGeneration, predecessorArmPid = ""): ArmResult {
    ...
    const armChild = spawn("bash", ["-lc", "config_dir=\"${FM_CONFIG_OVERRIDE:-$FM_HOME/config}\"; [ -f \"$config_dir/check-cadence.env\" ] && . \"$config_dir/check-cadence.env\"; exec \"$FM_WATCH_ARM_SCRIPT\" --restart"], {
      cwd: fmRoot,
      env,
