...
test_served_page_wears_dashboard_with_interactive_layer() {
  ...
  assert_contains "$RESP" 'data-dashboard-page="ideas"' "served page lacks its intentional Ideas page"
  assert_contains "$RESP" 'placeholder.replaceWith(ideasSection)' "served Ideas are not placed on the Ideas page"
...
test_your_go_controls_for_captain_awaiting_items() {
  ...
  assert_not_contains "$RESP" '.brief-card .rollcall li .why{display:none}' "narrow screens hide the generic decision controls"
