...
test_guard_warnings() {
  ...
  # arm-death marker from a prior silent cycle death.
...
test_arm_fails_loud_when_no_fresh_watcher_confirmable() {
  ...
  grep -F 'without an actionable reason' "$state/.watcher-arm-dead" >/dev/null \
...
# When an attached arm cycle ends with no healthy successor and no wake handoff,
# the death must be loud (typed FAILED line + durable marker + non-zero) so a
# supervision gap is still named on the next fleet action.
test_arm_attached_death_loud_when_tasks_in_flight() {
  ...
  [ "$(cat "$state/.watch.lock/pid" 2>/dev/null || true)" = "$wpid" ] || fail "seed watcher did not take the lock"
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_ARM_CONFIRM_TIMEOUT=1 FM_ARM_ATTACH_POLL=0.1 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF "watcher: attached pid=$wpid" "$armout" 2>/dev/null && break
  ...
  grep -F 'watcher: FAILED - cycle ended without an actionable reason' "$armout" >/dev/null \
  ...
  grep -F 'detail=attached arm cycle ended with no live successor and no wake handoff' "$state/.watcher-arm-dead" >/dev/null \
