...
- [ ] ordinary-orphan - Unowned release task (repo: sshhip) (kind: ship)' \
  ...
  # BSD sed's "a\" drops the newline that should follow the appended text, which
  # glues the next section header onto this row and silently breaks the fixture.
  # awk appends identically on both seds.
  awk '{ print }
       /## In flight/ { print "- [ ] unreadable-child - Submit App Store build (repo: sshhip) (kind: ship)" }' \
    "$sshhip/data/backlog.md" > "$sshhip/data/backlog.next"
  mv "$sshhip/data/backlog.next" "$sshhip/data/backlog.md"
