...
`bin/fm-supervision-lib.sh` also recognizes an armed inbound captain channel as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without an independent inbound-channel need, unaccounted primary work therefore reads as idle rather than suspicious.
