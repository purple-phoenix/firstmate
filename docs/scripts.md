...
| `fm-cadence.sh`          | Single owner of the watcher check cadence: reconcile `config/check-cadence.env` from this home's armed inbound captain channels |
| `fm-cadence-lib.sh`      | Shared check-cadence constants and generated-file body, for every process that locates or writes it |
