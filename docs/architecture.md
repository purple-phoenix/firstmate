...
For a terminal historical run only, a trailing `paused:` declaration wins when its status-file mtime is later than the entire minute represented by the matching branch-and-head row from `no-mistakes runs`, with a timestamp field from full `axi status` retained as a fallback; unavailable, ambiguous, or non-newer ordering leaves the terminal run authoritative.
