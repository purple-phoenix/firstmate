...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR/MR merge or GitHub dead-preview detection or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Preview wakes can be earned only by the canonical labeled tailnet declarations defined in `bin/fm-pr-poll.sh`; bare unlabeled URLs cannot earn one.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a matching-branch no-mistakes run, then keeps that run-step authoritative even if the pane has closed.
Parked and terminal runs require a resolvable current-code match, while full status for a run still progressing through `running`, `fixing`, or `ci` may attribute when its head object exists only in no-mistakes' isolated worktree and no gate or awaiting marker makes it parked.
...
When only an owned child's current classification is unavailable, or when an owned in-flight backlog row has a terminal child state (`done` or `failed`), the home classification stays unknown while independently trustworthy structured decisions, holds, queued and landed records, endpoint identities, counts, and provenance remain available; every other invalid path stays strict and exposes none of those child-derived surfaces.
Stale parent-event decisions never reappear as current captain decisions merely because that partial structured-home aggregation is degraded.
...
An attached cycle that ends after the durable wake sequence advanced is the watched holder's terminal wake being delivered, so it stays quiet and exits zero for the primary to re-arm on that notification.
Every `FAILED` path also writes `state/.watcher-arm-dead`, and a signal that ends supervision while tasks remain in flight writes the same marker, so `bin/fm-guard.sh` can name a supervision gap even when nothing was reading the arm's output.
...
That keeps spawn launch compatible across the verified crew harnesses owned by [`configuration.md`](configuration.md#harness-support) while preserving the requested profile for later audit.
...
`config/backlog-backend` is inherited too; [configuration.md](configuration.md#backlog-backend-taskstoml--configbacklog-backend) owns its selection behavior and validated exceptions.
...
On the locked session-start bootstrap step, that token creates the local polling artifact described in the [X mode configuration reference](configuration.md#x-mode-env), and arms the shared fast check cadence owned by the [watcher check cadence reference](configuration.md#watcher-check-cadence-configcheck-cadenceenv).
