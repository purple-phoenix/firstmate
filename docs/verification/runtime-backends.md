...
- Those distinctions hold byte-for-byte under `LC_ALL=C`, which is how the no-mistakes suite runs: before the exact-prefix matching landed, byte-counting `${var#?}` and `${var:0:1}` misread post-glyph idle placeholders as pending, promoted box-drawing rows to bare composers, and classified U+2063 injections as user input.
...
`tests/fm-composer-lib.test.sh` additionally pins the locale-invariant post-glyph idle placeholders, and `tests/fm-backend-herdr.test.sh` pins custom-regex matching under a discovered UTF-8 locale plus the unknown-rather-than-guess path when no UTF-8 locale is available.
