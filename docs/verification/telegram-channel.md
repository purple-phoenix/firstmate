...
- **X mode and the dashboard channel**: independently armable, each keeping its own check, records, and wake vocabulary.
  X mode's own artifact is now just its relay poll shim: the cadence it used to write moved to the shared owner, which every armed channel shares; [`dashboard-service.md`](../dashboard-service.md#verification-evidence) owns current dashboard command-and-chat verification.
