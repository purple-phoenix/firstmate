...
Composer matching is locale-invariant.
The default `❯` and `›` glyphs are matched as exact shell prefixes rather than through a multibyte regular-expression character class, and the shared classifier removes each full glyph prefix explicitly, so those rows and any post-glyph idle placeholder keep the same verdict under `LC_ALL=C`.
An overridden `FM_BACKEND_HERDR_BARE_PROMPT_RE` runs only under a discovered UTF-8 locale and otherwise leaves the row unknown rather than guessing.
[`configuration.md`](configuration.md) owns that setting's contract.
