...
3. The plugin listens for `session.idle`, spawns `bin/fm-watch-arm.sh --restart` without awaiting it in the idle handler, relies on that launch owner to apply the validated cadence without sourcing config bytes, and owns every later successor launch.
