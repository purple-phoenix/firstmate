...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and optional scout supporting evidence.
...
Dispatchable item creation is a guarded exception to backend selection: `bin/fm-task-add.sh` validates or fits the task ID before delegating creation to `tasks-axi`, regardless of `config/backlog-backend`.
...
Because bootstrap requires `tasks-axi` on `PATH` on every profile, those delegations work fleet-wide.
...
Set the local, gitignored `config/backlog-backend` file to `manual` to force manual editing for routine backlog mutations other than guarded dispatchable creation and suppress the verbose `BOOTSTRAP_INFO: tasks-axi available` fact, not missing-tool reporting.
...
A writable install also serves the captain chat destination and the exact PR merge approval flow owned by [`dashboard-service.md`](dashboard-service.md): chat records live under mode-0700 `state/dash-chat/` (`messages/`, `archive/`, and the one-reply-per-message `replies/` ledger consumed and written through `bin/fm-dash-chat.sh`), merge approvals ride `state/dash-inbox/` as typed `kind=merge-approval` records, and their one-time consumption ledger lives under `state/dash-merge/consumed/` (written only by `bin/fm-dash-merge.sh`).
...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^[❯›]'  # herdr-only: the default recognizes claude's ❯ and codex's › as UNBORDERED (bare) composer rows with locale-invariant exact-prefix matching; a custom regex runs only under a discovered UTF-8 locale and otherwise stays unknown rather than guessing; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Composer and injection safety")
...
FM_TG_HTTP_TIMEOUT=     # seconds allowed per Bot API call; clamped to 1..28, unset defaults to FM_TG_POLL_TIMEOUT + 8, and 20 elsewhere
