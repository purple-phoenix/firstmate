...
The dashboard renders self-clearing waits (a running test suite or validation, CI checks, a declared external delay, or a future time gate) as a calm resumes-by-itself treatment with honest progress, distinct from blocked work that needs an actor, and a declared pause whose evidence shows the wait is on the captain renders as needs-your-action with no ETA; `bin/fm-wait-progress.mjs` owns the estimator and the producer-owned private rolling record `data/capacity-wait-history.json` (`fm-capacity-wait-history.v1`) it estimates from, and `bin/fm-capacity.mjs --help` owns the honesty rules.
