...
The dashboard renders self-clearing waits (a running test suite or validation, CI checks, a declared external delay, or a future time gate) as a calm resumes-by-itself treatment with honest progress, distinct from blocked work that needs an actor; `bin/fm-wait-progress.mjs` owns the estimator and the producer-owned private rolling record `data/capacity-wait-history.json` (`fm-capacity-wait-history.v1`) it estimates from, and `bin/fm-capacity.mjs --help` owns the honesty rules.
...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^[❯›]'  # herdr-only: the default recognizes claude's ❯ and codex's › as UNBORDERED (bare) composer rows with locale-invariant exact-prefix matching; a custom regex runs only under a discovered UTF-8 locale and otherwise fails closed as unknown; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Composer-emptiness safety", "Incident (2026-07-08)", "Incident (2026-07-10)")
