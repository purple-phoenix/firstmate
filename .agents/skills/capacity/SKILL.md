...
On a `check:` wake naming `fm-dash.check.sh`, run `bin/fm-dash-inbox.sh claim` and `bin/fm-dash-chat.sh claim`, then handle each claimed record by its kind:
...
- A `your-go` record is the captain's verdict on an item the dashboard listed as awaiting them, carried in `action`: `go` means lift a captain hold through the normal backlog lifecycle where one is set and re-resolve next steps through normal re-evaluation, dispatch, and authority checks; `park` means park the item through the normal backlog lifecycle so it rests until the captain returns to it; `guidance` carries bounded captain text - treat it as captain input on that item, and when the record names a `decision_identity`, route it through `decision-hold-lifecycle` exactly as a chat answer; work owned by a secondmate is routed to that home rather than edited from the main home.
- A `merge-approval` record is the captain's exact-merge word for one reviewed PR version, but ONLY through the guarded consumer: run `bin/fm-dash-merge.sh <task-id>` and let it validate the record, consume its one-time nonce, recheck the live PR, and merge through the guarded owner.
  Never read the record's fields and merge directly, never bypass or retry around a refusal, and treat its refusal, invalidation, or failed-attempt outcome as the result to report; a fresh approval requires a fresh dashboard review.
- A record of any other or unknown kind grants nothing: refuse it and route the ask to captain chat as trusted discussion.

Handle claimed chat messages under the same discipline as the Telegram channel: each message is captain input classified as a question, an instruction, or a decision; act through the normal lifecycle first, then record exactly one outcome-first, phone-short answer per message with `bin/fm-dash-chat.sh reply <message_id> --text-file <path>` (never shell interpolation), keeping section 9's translation rule and sending long material as tailnet links.
The reply ledger refuses a second answer, so a re-surfaced message is safe to re-read but never re-answered.
Never solicit or send a credential, key, token, or recovery code through chat; name what is needed and route it to the terminal or keychain.

Outside the validated `merge-approval` path above, a claimed record never authorizes a PR merge, `local-only` landing, destructive action, irreversible action, security-sensitive action, or discard of unlanded work; when a claimed action leads to such a choice, escalate it to captain chat exactly as section 4 requires.
...
While the service is installed and registered, the dashboard is an armed inbound captain channel: keep the live supervision cycle running even with no other fleet work so a click or chat message can wake Firstmate on the fast cadence.
